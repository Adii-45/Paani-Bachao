...
| Storage sizing | Indian Railways Institute of Civil Engineering (IRICEN) | *Rain Water Harvesting* (November 2022), §2.2.8.1, pages 41-42 | July-to-June monthly yield less constant monthly demand; maximum positive cumulative surplus is storage capacity | Named strategy using 12 IMD monthly normals and explicit user demand; otherwise unavailable | High for published method; normal-year and monthly-resolution limitations are exposed |
...
| Monthly rainfall distribution | IMD | *All India Districtwise Rainfall Normals (1971-2020)*, January-December layers | Twelve district monthly normals for the same 50-year period | All monthly layers imported and cross-validated against annual feature IDs; used only by the named IRICEN normal-year strategy | High for source/provenance; normals do not establish year-to-year reliability |
...
- IMD January district normals product (same pattern for all months): https://www.imdpune.gov.in/climinfo/normals/jan/index.html
- IRICEN, *Rain Water Harvesting* (November 2022): https://iricen.gov.in/iricen/books_jquery/rain_water_harvesting.pdf
...
The demo rules are not suitable as a default or fallback and are removed. The replacement engine calculates annual rooftop harvest only when both an official rainfall record and a matching Table 7.2 roof coefficient are available. Storage overflow can become recharge-available water only through the completed water balance. Feasibility, structure selection and sizing return explicit insufficient-data or unsupported states until their source-specific inputs are present. Installed structure strategies are limited to reviewed Hauz Khas CGWB Delhi coverage and reviewed Jayanagar CGWB/KSCST coverage.
